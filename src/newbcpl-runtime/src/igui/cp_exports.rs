//! Native shims for iGui. Borrowed from the sister NewCormanLisp
//! repo (which in turn ported them from NewCP, where they were
//! installed via a NativeModuleArtifact). Here they're plain Rust
//! `extern "C"` functions; the BCPL compiler / loader wires them
//! up as foreign symbols the same way it does any other runtime
//! built-in.
//!
...
//! sub-modules (`window`, `channels`, `batch`, `dwrite`). When the
//! BCPL JIT lands, BCPL-facing wrappers will live alongside the
//! other runtime shims; for now this language-neutral C-ABI is
//! the iGui surface.
//!
//! The filename retains its historical `cp_exports` name to stay
//! diff-comparable with the sister repos; rename is mechanical when
//! we want it.
