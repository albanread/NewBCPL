...
const FRAME_CLASS: PCWSTR = w!("NewBCPL.iGui.Frame");
    ...
    F: FnOnce() + Send + 'static,
{
    ...
    // Auto-open bedit and the log view as soon as the frame is up —
    // bedit is the user's primary surface, and the log view shows
    // stdout for programs invoked via `Program ▸ Run`. The user can
    // close either; they re-open via Tools or the accelerator.
    super::bedit::open(hwnd, mdi);
    super::log_view::open(hwnd, mdi);
    // Restore bedit's focus so the user can start typing immediately
    // (log_view::open activated itself last otherwise).
    super::bedit::open(hwnd, mdi);
    ...
    // Ctrl+Shift+E opens bedit, Ctrl+Shift+L opens the log view,
    ...
    let exit_code = unsafe {
        loop {
            ...
            // open bedit) regardless of which child has focus.
            if let Some(h) = accel {
...
unsafe extern "system" fn frame_wnd_proc(
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            // Built-in tools (bedit, log view) are wired before the
            // user menu so they work even if no language-thread spec
            // has been installed.
            if cmd_id == super::bedit::MENU_CMD_ID {
                if mdi.0 as isize != 0 {
                    super::bedit::open(hwnd, mdi);
                }
                return LRESULT(0);
