//! bedit — fail-safe BCPL-aware editor.
//!
...
//! any BCPL code being loaded. The point is that even when the
//! rest of NewBCPL has a fault, the editor remains responsive so
...
/// WM_COMMAND id for the "Tools > bedit" frame menu entry. Outside
...
/// WndProc consumes the box. Posted by `load_file_into_singleton`
/// from the same thread that owns the window (the UI thread).
const WM_BEDIT_LOAD_FILE: u32 =
    windows::Win32::UI::WindowsAndMessaging::WM_USER + 1;

/// Private WM_USER message: snapshot the current buffer as a
/// String. `lparam` points at a `Mutex<Option<String>>` the WndProc
/// fills in. Sent (not posted) from the language thread by
/// `snapshot_buffer`, so when SendMessageW returns the cell is
/// already populated.
...
/// file is the buffer's initial content; Ctrl+S then writes back
/// to that same path, and `Program ▸ Run` reads from it.
static STARTUP_FILE: Mutex<Option<PathBuf>> = Mutex::new(None);

/// Register a file path that the first-created bedit instance will
/// load into its buffer at startup, also setting `file_path` so
/// Ctrl+S saves back to it. Call once before `igui::run`; consumed
/// (taken) the first time `bedit::open` succeeds.
pub fn set_startup_file(path: PathBuf) {
...
pub fn snapshot_buffer() -> Option<String> {
    ...
    unsafe {
        SendMessageW(
            hwnd,
            WM_BEDIT_SNAPSHOT_BUFFER,
            Some(WPARAM(0)),
            Some(LPARAM(cell_ptr)),
        );
    }
    // SendMessageW returned, so the UI thread's WndProc has filled
    // the cell. Borrow gone; safe to take the value out.
    cell.into_inner().expect("snapshot cell poisoned")
}

// ─── Compile-check injection point ──────────────────────────────────
//
// The runtime crate sits below `newbcpl-parser` and `newbcpl-sema`
// in the dependency graph, so it cannot import them directly.
// Instead the driver (which already depends on both) hands bedit a
// closure that runs a check and returns diagnostics. This keeps the
...
/// Register the bedit MDI child WndClass. Called from
/// `child::register_classes`.
pub fn register_class() -> Result<(), super::IGuiError> {
    let h_instance = unsafe { GetModuleHandleW(None) }
        .map_err(|e| super::IGuiError::Win32(format!("GetModuleHandleW (bedit): {e}")))?
        .into();
    let cursor = unsafe { LoadCursorW(None, IDC_IBEAM) }
        .map_err(|e| super::IGuiError::Win32(format!("LoadCursorW (bedit): {e}")))?;
    let cls = WNDCLASSEXW {
        cbSize: std::mem::size_of::<WNDCLASSEXW>() as u32,
        style: WNDCLASS_STYLES(0),
        lpfnWndProc: Some(bedit_wnd_proc),
        cbClsExtra: 0,
        cbWndExtra: 0,
...
// `tools_menu`, which knows about both bedit and the log view.

/// Open the bedit child, or activate it if already open. Called from
...
pub fn open(frame: HWND, mdi_client: HWND) {
    ...
    let h_instance = match unsafe { GetModuleHandleW(None) } {
        ...
        Err(e) => {
            eprintln!("[bedit] GetModuleHandleW: {e}");
            return;
        }
    };
    ...
    if result.0 == 0 {
        eprintln!("[bedit] WM_MDICREATE returned 0");
        return;
    }
...
struct ReditState {
    ...
    /// buffer on every edit is fine for the sub-MB files bedit is
    /// designed to handle.
...
impl ReditState {
    ...
    fn ensure_resources(&mut self, w: u32, h: u32) {
        ...
        match target {
            ...
            Err(e) => eprintln!("[bedit] CreateHwndRenderTarget failed: {e}"),
        }
    }
    ...
    /// buffer. No-op when no checker is installed — keeps bedit
    ...
    fn paint(&mut self) {
        ...
        let braces_segment = match self.brace_balance() {
            0 => "{}".to_string(),
            n if n > 0 => format!("{{+{n}}}"),
            n => format!("{{{n}}}"),
        };
    ...
    fn insert_newline(&mut self) {
        ...
        // and add two extra spaces if the cursor sits inside an open
        // block — either there's an unmatched `{` / `$(` on this line
        // to the left of the cursor, or the line (up to the cursor)
        // ends in a BCPL block-opening keyword (`DO`, `THEN`, `ELSE`,
        // `BE`). Cheap heuristic; good enough for hand-written BCPL
        // without going through a full parser. Strings, char literals
        // and `//` line comments are skipped so braces inside them
    ...
    fn compute_auto_indent(&self) -> String {
        ...
        let leading: String = line
            ...
            .take_while(|&c| c == ' ' || c == '\t')
            .collect();
        // Scan the line up to the cursor counting BCPL block openers
        // (`{` and `$(`) minus closers (`}` and `$)`). Skip text
        // inside `"…"` and `'…'` literals (with `*` as the escape
        // character — BCPL convention) and inside `//` line comments.
        // Block comments `/* … */` are intentionally handled per
        // line only here; the multi-line balance counter below threads
        // the comment state.
        ...
        let mut i: usize = 0;
        let limit = self.cursor_col.min(chars.len());
        while i < limit {
            let c = chars[i];
            if in_string {
                if after_escape {
                    ...
                    // BCPL string escape — consume the next char along
                    // with the `*` so a literal `*"` doesn't close the
                    // string.
                    after_escape = true;
        ...
        // A trailing block-opener keyword counts as one more level
        // of indent — e.g. `IF p THEN`, `WHILE q DO`, `LET f() BE`.
        // Look at the line up to the cursor with trailing whitespace
        // stripped.
        let prefix: String = chars[..limit].iter().collect();
        let trimmed = prefix.trim_end();
        if depth <= 0 && trailing_block_opener(trimmed) {
            depth = 1;
        }
        if depth > 0 {
    ...
    /// Buffer-wide brace count: opens minus closes for both `{ } `
    /// and the BCPL section-bracket synonyms `$( $)`. Strings,
    /// char literals, `//` line comments, and `/* … */` block
    /// comments are treated as opaque. 0 = balanced; positive =
    /// more opens than closes; negative = more closes (likely an
    /// editing accident).
    fn brace_balance(&self) -> i32 {
        let mut depth: i32 = 0;
        let mut in_string = false;
        let mut string_quote = '"';
        let mut after_escape = false;
        ...
        for line_idx in 0..self.line_count() {
            ...
            while i < chars.len() {
                ...
                if c == '/' && i + 1 < chars.len() {
                    if chars[i + 1] == '/' {
                        // Line comment runs to end of line — skip out.
                        break;
                    }
                    if chars[i + 1] == '*' {
            ...
            // Newline ends a `//` line comment and a string literal
            // (BCPL strings do not span lines); block comments
    ...
    fn load_from(&mut self, path: PathBuf) {
        match std::fs::read(&path) {
            ...
            Err(e) => eprintln!("[bedit] read {path:?} failed: {e}", path = self.file_path),
        }
    }
    ...
    fn update_title(&self) {
        ...
        let title = format!("bedit — {name}{star}", star = if self.dirty { " *" } else { "" });
...
unsafe extern "system" fn bedit_wnd_proc(
    hwnd: HWND,
    msg: u32,
    wparam: WPARAM,
    lparam: LPARAM,
    ...
    // Custom WM_USER messages — not part of the match below because
    // `match` requires literal patterns and `WM_USER + N` isn't one.
    if msg == WM_BEDIT_LOAD_FILE {
        if lparam.0 != 0 {
            // SAFETY: lparam came from a Box::into_raw on a PathBuf,
            // posted via SendMessageW from `load_file_into_singleton`.
            // We're the sole consumer of this message and the Box is
    ...
    if msg == WM_BEDIT_SNAPSHOT_BUFFER {
        if lparam.0 != 0 {
            // SAFETY: lparam points at a stack-allocated
            // `Mutex<Option<String>>` owned by the caller. Because
            // we got here via SendMessageW (synchronous), the
...
// ─── BCPL tokenizer (R3 syntax highlighting) ───────────────────────
...
/// BCPL reserved words. Kept in sync with `newbcpl-lexer::KEYWORDS`;
/// drift just means a few keywords colour as identifiers, never a
/// correctness bug for the editor.
...
fn is_bcpl_keyword(word: &str) -> bool {
    // BCPL keywords are case-sensitive upper-case per the canonical
    // convention. Linear scan — the table is small and runs once per
    // identifier.
    BCPL_KEYWORDS.iter().any(|k| *k == word)
}

/// Does `prefix` (the current line up to the cursor, with trailing
/// whitespace already stripped) end in a BCPL block-opening keyword?
/// Used by auto-indent so `IF cond THEN\n` indents the next line.
fn trailing_block_opener(prefix: &str) -> bool {
    // Walk back over alphanumerics to extract the trailing word.
...
/// Tokenize one line, given whether we entered the line already
/// inside a `/* … */` block comment. Returns the produced tokens
/// and the carry-state for the next line.
fn tokenize_line(line: &str, in_block_in: bool) -> (Vec<Token>, bool) {
    let chars: Vec<char> = line.chars().collect();
    let mut tokens = Vec::new();
    let mut i = 0usize;
    let mut in_block = in_block_in;

    // If we entered already inside a block comment, scan for the
    // closing `*/` first.
    if in_block {
        let start = 0;
        while i < chars.len() {
    ...
    while i < chars.len() {
        ...
        // String / char literal — BCPL uses `*` as the escape lead,
        // not `\`. Both quote styles stay on one line.
        if c == '"' || c == '\'' {
            let quote = c;
            let start = i;
            i += 1;
            while i < chars.len() {
                let ch = chars[i];
                if ch == '*' && i + 1 < chars.len() {
                    // Skip the escape lead and its following char.
                    i += 2;
                    continue;
                }
                if ch == quote {
        ...
        // Hash-led number — `#X1A` hex or `#777` octal.
        if c == '#' && i + 1 < chars.len() {
            let next = chars[i + 1];
            if next.is_ascii_digit()
                || next == 'X' || next == 'x' || next == 'O' || next == 'o'
                || next == 'B' || next == 'b'
            {
                let start = i;
                i += 1;
        ...
        // Decimal / real number — `123`, `1.5`, `1.5e2`, `2E-3`.
        if c.is_ascii_digit() {
            let start = i;
            ...
            if i < chars.len() && (chars[i] == 'e' || chars[i] == 'E') {
                ...
                if i < chars.len() && chars[i].is_ascii_digit() {
                    ...
                    i = save; // not really an exponent — back off
                }
            }
            tokens.push(Token {
                start,
        ...
        // Anything else (operators, whitespace, punctuation, the bare
        // `$` of `$( $)` section brackets) is left unstyled.
        i += 1;
    }
...
fn clipboard_set(owner: HWND, text: &str) -> bool {
    ...
    unsafe {
        ...
        match GlobalAlloc(GMEM_MOVEABLE, bytes) {
            ...
            Err(e) => eprintln!("[bedit] GlobalAlloc failed: {e}"),
        }
